...
SHARED=/vast/draco/tara/projects/Tara_Deployment/software/pd_smoke_${STAMP}
mkdir -p ${SHARED}/logs
...
cleanup() {
    ...
    kill -TERM ${P_SSH_PID} ${D_SSH_PID} ${TAIL_P_PID} ${TAIL_D_PID} 2>/dev/null
    sleep 5
...
# =============================================================================
# PROXY -- still a placeholder. Wire in the actual reference proxy once you've
# pulled it from a matching vllm source checkout (tests/v1/kv_connector/
# nixl_integration/), and point its host args at the hsn0 IPs below, not
# localhost or plain hostnames.
# =============================================================================
# ssh -n "${NODE_P}" "source ${SHARED}/common_env.sh && \
#     python3 <path-to>/toy_proxy_server.py \
#     --port ${PROXY_PORT} \
#     --prefiller-host ${P_IP} --prefiller-port ${P_PORT} \
#     --decoder-host ${D_IP} --decoder-port ${D_PORT}" \
#     > ${SHARED}/logs/proxy.log 2>&1 &
# PROXY_SSH_PID=$!
# touch ${SHARED}/logs/proxy.log
# tail -n +1 -f ${SHARED}/logs/proxy.log | sed -u 's/^/[PROXY] /' &
# TAIL_PROXY_PID=$!
# wait_healthy ${P_IP} ${PROXY_PORT} "Proxy (${NODE_P})" || exit 1
# (also add PROXY_SSH_PID and TAIL_PROXY_PID to the kill list in cleanup() above)

echo ""
echo "=== PROXY NOT YET WIRED IN -- Q1b/Q2/Q3 below are inert until it is. ==="
echo "Once the proxy block above is uncommented, this section will:"
echo "  1. Snapshot hsn0/bond0 byte counters on both nodes (Q2)"
echo "  2. Send the same prompt through the proxy, timed the same way (Q1b)"
echo "  3. Snapshot the counters again and diff them"
echo ""
