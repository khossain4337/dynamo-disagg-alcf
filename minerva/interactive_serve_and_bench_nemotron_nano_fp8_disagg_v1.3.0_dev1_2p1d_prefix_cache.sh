...
#
# prefix_cache variant changes from v1_3_0:
#   - --no-enable-prefix-caching REMOVED from all workers
#   - Requires patch applied via vllm_nixl_assert_patch.sh
#     Patch replaces assert num_local_blocks == num_remote_blocks
#     with trim-to-minimum for SSM groups in nixl/worker.py
#     _apply_prefix_caching. Without the patch, 2+ prefill workers
#     crash with AssertionError on every request.
# ==========================================
...
SCENARIOS=(
    #"16384,256,500,10,32"
    "16384,2048,200,5,32" ## avoiding potential OOM
    #"8192,256,500,10,32"
    #"32768,256,200,5,32"
)
...
mkdir -p "$RESULTS_DIR"
RUN_ID="$(tstamp)"
FRONTEND_LOG="$RESULTS_DIR/disagg_2p1d_prefix_cache_frontend_${RUN_ID}.log"
DECODE_LOG="$RESULTS_DIR/disagg_2p1d_prefix_cache_decode_${RUN_ID}.log"
PREFILL_LOG_1="$RESULTS_DIR/disagg_2p1d_prefix_cache_prefill1_${RUN_ID}.log"
PREFILL_LOG_2="$RESULTS_DIR/disagg_2p1d_prefix_cache_prefill2_${RUN_ID}.log"
touch "$FRONTEND_LOG" "$DECODE_LOG" "$PREFILL_LOG_1" "$PREFILL_LOG_2"

echo "=========================================="
echo "Dynamo Disaggregated Serving 2P:1D — Nemotron Nano FP8"
echo "Dynamo version: 1.3.0-dev.1 (prefix_cache variant)"
echo "=========================================="
echo "Run ID:        $RUN_ID"
echo "Model:         $MODEL"
...
echo "Prefix cache:  ENABLED (patch applied)"
...
for SCENARIO in "${SCENARIOS[@]}"; do
    ...
    echo "=========================================="
    echo "Benchmarking scenario (DISAGG 2P:1D prefix_cache):"
    ...
    RESULT_FILE="$RESULTS_DIR/DISAGG_2P1D_PREFIX_CACHE__nemotron_nano_fp8__in${INPUT_LEN}_out${OUTPUT_LEN}__prm${NUM_PROMPTS}__rate${REQUEST_RATE}__seq${MAX_CONCURRENCY}__${RUN_ID}.json"
...
# ==========================================
echo "=========================================="
echo "All 2P:1D prefix_cache disaggregated benchmarks complete!"
echo "Results:"
ls -lh "$RESULTS_DIR"/DISAGG_2P1D_PREFIX_CACHE__* 2>/dev/null || echo "No DISAGG_2P1D_PREFIX_CACHE result files found"
echo "=========================================="
