...
# =============================================================================
# Nemotron Ultra NVFP4 — Dynamo DISAGG 2P:2D — OSL Sweep — Single Entry Point
# Run this script ONLY on minerva-dgx-01. It SSHes to node2 automatically.
#
# USAGE:
#   bash interactive_single_entry_multinode_ultra_disagg_kv_aware_osl_sweep_2p2d_N2.sh
#
# WHAT IT DOES:
...
#   4. Sweeps OSL: 256 → 4096 → 8192 → 16384 (ISL fixed at 131,072)
#      At each OSL step: restarts all workers with new MAX_MODEL_LEN + correct
#      MAX_NUM_SEQS, then runs a small benchmark probe
#   5. Stops at the first OSL where median TTFT > TTFT_LIMIT_MS or startup OOM
...
# =============================================================================
# SECTION 3 — Fixed configuration (does NOT change between OSL steps)
# =============================================================================
...
# Adjust based on your tolerance. Comment out to run all OSL steps regardless.
...
# -------------------------------------------------------------------------
#
# OSL SWEEP DESIGN (ISL fixed at 131,072 — from Phase 1 anchor):
#
# [FACT — from Phase 1 fit] TTFT = 71.09ms × ISL/1000 + 2,595ms
#   At ISL=131,072: TTFT ≈ 11,914ms ≈ 11.9s
#   At rate=0.05: ρ = 0.05 × 11.9 / 2 = 0.30 → low utilization, TTFT flat
#
# Expected findings:
#   TTFT  ≈ flat across all OSL (prefill is ISL-only, confirmed by Phase 1)
#   TPOT  ≈ flat at ~7ms (memory-BW floor — ISL-independent)
#   Total = TTFT + OSL × TPOT scales linearly with OSL
#
# Decode time estimates at TPOT=7ms:
#   OSL=256   → decode =   256 × 7ms =    1.8s  → total req time ~13.7s
#   OSL=4096  → decode =  4096 × 7ms =   28.7s  → total req time ~40.6s
#   OSL=8192  → decode =  8192 × 7ms =   57.3s  → total req time ~69.2s
#   OSL=16384 → decode = 16384 × 7ms =  114.7s  → total req time ~126.6s
#
# max_model_len per step = ISL + OSL (all safe under 262,144 ceiling):
#   OSL=256   → max_model_len = 131,328
#   OSL=4096  → max_model_len = 135,168
#   OSL=8192  → max_model_len = 139,264
#   OSL=16384 → max_model_len = 147,456  ← stretch point, still well under ceiling ✓
#
# =============================================================================
...
ISL_SWEEP=(
    ...
    "131072,4096,32,5,0.05,4"
    "131072,8192,32,5,0.05,4"
    "131072,16384,32,5,0.05,4"
)
...
mkdir -p "$RESULTS_DIR"
RUN_ID="$(tstamp)"
SWEEP_SUMMARY="$RESULTS_DIR/ULTRA_DISAGG_2P2D_KV_2NODE__osl_sweep_summary_${RUN_ID}.txt"

log "=========================================="
log "Nemotron Ultra NVFP4 — DISAGG 2P:2D OSL Sweep (ISL=131072 fixed)"
...
log "OSL steps: ${#ISL_SWEEP[@]}"
log "ISL: 131072 (fixed)"
...
#             These stay up for the ENTIRE sweep — no restart between OSL steps
# =============================================================================
...
#
# For each ISL step:
#   a) Kill and restart all 4 workers with new MAX_MODEL_LEN for this OSL step
...
echo "" >> "$SWEEP_SUMMARY"
echo "OSL Sweep — Nemotron Ultra NVFP4 DISAGG 2P:2D — Run $RUN_ID" >> "$SWEEP_SUMMARY"
echo "ISL=131072 (fixed), TTFT_LIMIT=${TTFT_LIMIT_MS}ms" >> "$SWEEP_SUMMARY"
...
for STEP in "${ISL_SWEEP[@]}"; do
    ...
    log ""
    log "=========================================="
    log "OSL STEP: OSL=$OSL  ISL=$ISL  MAX_NUM_SEQS=$MAX_NUM_SEQS"
    ...
    # ------------------------------------------------------------------
    log "Stopping any running workers from previous OSL step..."
    kill_leftovers_local
    kill_leftovers_remote
...
ENDSSH
    ...
    log ""
    log "--- Benchmark probe: OSL=$OSL  ISL=$ISL  NUM_PROMPTS=$NUM_PROMPTS  RATE=$RATE ---"
...
" 2>/dev/null || echo "-1")
    ...
    log "Verdict for OSL=$OSL: $VERDICT"
