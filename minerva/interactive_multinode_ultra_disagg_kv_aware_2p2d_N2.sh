...
# Router modes to run sequentially in the same session.
# [FACT] Default in Dynamo is round-robin if not set explicitly.
# [FACT] Set via DYN_ROUTER_MODE env var on the frontend process.
# [GUESS] kv router may prefer same-node prefill→decode pairs (NVLink)
#         over cross-node pairs (IB). Needs source verification before trusting.
# Running both back-to-back with same workers — fair comparison, no restart needed.
ROUTER_MODES=(
    "round-robin"   # apples-to-apples baseline vs 2P:1D
    "kv"            # KV-cache-aware — may exploit NVLink topology
)

# Benchmark scenarios
# [FACT] 2P:1D decode-heavy saturated at rate=5 (0.89 effective req/s, peak=54).
# Start at rate=5 to compare directly. Then rate=10 prefill-heavy.
SCENARIOS=(
...
run_node1() {
    ...
    # --- Frontend started later, once per router mode (see benchmark loop below) ---
    ...
    # --- Wait for frontend ---
    # (frontend launched inside router mode loop below)

    echo "Allowing all 4 workers to fully register in etcd (30s)..."
    sleep 30
    echo "All workers registered. Starting benchmark runs."
    echo ""

    # --- Benchmarks — outer loop over router modes ---
    # [FACT] Workers stay running between router mode switches — no restart needed.
    # Only the frontend is restarted to pick up the new DYN_ROUTER_MODE.
    # [FACT] DYN_ROUTER_MODE env var controls Dynamo frontend routing strategy.
    # [GUESS] kv router may prefer same-node prefill→decode pairs (NVLink fast path).
    #         Verify from source before trusting this assumption.
    for ROUTER_MODE in "${ROUTER_MODES[@]}"; do

        echo "=========================================="
        echo "Starting frontend with router mode: $ROUTER_MODE"
        echo "=========================================="

        # Kill previous frontend if this is a restart for a new router mode.
        # [FACT] pkill alone is not enough — the old frontend holds port 8000
        # while draining in-flight responses. Must wait for the port to be
        # fully released before launching the new frontend, or the new process
        # crashes immediately with EADDRINUSE (os error 98).
        pkill -f "dynamo.frontend" 2>/dev/null || true

        echo "Waiting for port $HTTP_PORT to be released..."
        local port_wait_start=$SECONDS
        while (( SECONDS - port_wait_start < 30 )); do
            # ss -tlnp: TCP listening sockets. If port 8000 not in output, it's free.
            if ! ss -tlnp 2>/dev/null | grep -q ":${HTTP_PORT} "; then
                echo "Port $HTTP_PORT free after $(( SECONDS - port_wait_start ))s ✓"
                break
            fi
            sleep 1
        done
        # Extra safety margin — OS may not immediately accept new bind even after
        # ss shows port free (TIME_WAIT state, kernel backlog drain).
        sleep 3

        # [FACT] kv router with random prompts (no prefix reuse):
        #   DYN_ROUTER_ASSUME_KV_REUSE=false — generates random hashes instead
        #   of assuming cache reuse. Tells the router our workload has no prefix
        #   sharing, so it falls back to pure load-based routing (active blocks +
        #   prefill tokens). Without this, kv router assumes reuse and wastes
        #   cycles computing overlap scores that will always be 0.
        # [FACT] round-robin ignores all kv router flags — they are no-ops.
        ...
        FRONTEND_PID=$!
        echo "Frontend PID: $FRONTEND_PID (router: $ROUTER_MODE)"

        wait_for_worker \
            "http://127.0.0.1:${HTTP_PORT}/health" \
            "Dynamo frontend ($ROUTER_MODE)"

        # Extra settle — workers re-register endpoints with new frontend instance
        echo "Allowing workers to re-register with new frontend (15s)..."
        sleep 15
        ...
        for SCENARIO in "${SCENARIOS[@]}"; do
            ...
            # Router mode embedded in filename for easy comparison
            local SAFE_ROUTER
            SAFE_ROUTER=$(echo "$ROUTER_MODE" | tr '-' '_')
            local RESULT_FILE
            RESULT_FILE="$RESULTS_DIR/ULTRA_DISAGG_2P2D_2NODE__${SAFE_ROUTER}__in${INPUT_LEN}_out${OUTPUT_LEN}__prm${NUM_PROMPTS}__rate${REQUEST_RATE}__seq${MAX_CONCURRENCY}__${RUN_ID}.json"
            ...
            echo "Done: router=$ROUTER_MODE input=$INPUT_LEN output=$OUTPUT_LEN"
            echo "Results: $RESULT_FILE"
            echo ""
        done

        echo "Router mode '$ROUTER_MODE' complete."
        echo ""

    done  # end router mode loop
