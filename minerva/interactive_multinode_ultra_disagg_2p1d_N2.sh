...
# ==========================================
# Nemotron Ultra NVFP4 — Dynamo DISAGG 2P:1D Multi-Node
...
#
# USAGE:
#   Terminal 1 (minerva-dgx-01): bash ultra_disagg_2p1d_multinode.sh
#   Terminal 2 (minerva-dgx-02): bash ultra_disagg_2p1d_multinode.sh
...
#     Prefill-1   → GPUs 0,1,2,3  DYN_SYSTEM_PORT=8082  NUMA node 0
#
#   minerva-dgx-02 (NODE2):
#     Prefill-2   → GPUs 0,1,2,3  DYN_SYSTEM_PORT=8083  NUMA node 0
#
# KV transfer: NIXL over InfiniBand (inter-node, mlx5 verbs)
#
# [FACT] etcd binds to 0.0.0.0 and advertises NODE1_IP so node2 can reach it.
#        Single-node scripts used 127.0.0.1 — that would be invisible to node2.
# [FACT] no_proxy includes both node IPs — without this, the ALCF proxy
#        intercepts etcd/NATS TCP silently and breaks worker registration.
# [FACT] ETCD_ENDPOINTS and NATS_SERVER set explicitly on every worker launch,
#        not just in shell env — numactl subprocess may not inherit all exports.
# [FACT] Decode starts first — prefill workers load decode metadata from etcd.
# [FACT] NIXL_SIDE_CHANNEL_PORT unique per prefill worker: 20097 (P1), 20098 (P2).
# [FACT] KV_EVENTS_PORT unique per prefill worker: 20081 (P1), 20082 (P2).
# [FACT] No --kv-cache-dtype — Ultra auto-selects fp8_e4m3.
# [FACT] No VLLM_USE_FLASHINFER_MOE_FP8 — Ultra uses FP4 MoE kernel.
# [FACT] VLLM_SSM_CONV_STATE_LAYOUT=DS required for Mamba-2 KV transfer.
# [FACT] --dyn-reasoning-parser nemotron_v3 — NOT ultra_v3 (not a built-in).
# [FACT] SSM patch must be applied (once, on node1 — NFS propagates to node2).
#        Verify: grep "num_blocks = min" \
#          ~/software/envs/conda_envs/dynamo_vllm_1.3.0_dev1/lib/python3.12/ \
#          site-packages/vllm/distributed/kv_transfer/kv_connector/v1/nixl/worker.py
# [GUESS] NIXL binds kv-events on tcp://*:port — advertises a reachable IP
#         to decode via etcd. Assumed to pick the correct IB interface.
#         Watch first-request logs to confirm inter-node KV transfer succeeds.
# ==========================================
...
NODE1_IP="10.125.13.11"    # [FACT] vlan2257, MTU 9000, confirmed by ip addr
NODE2_IP="10.125.13.12"    # [FACT] vlan2257, MTU 9000, confirmed by ip addr
...
export https_proxy=http://proxy.alcf.anl.gov:3128
export http_proxy=http://proxy.alcf.anl.gov:3128
# [FACT] Must include node IPs — proxy must not intercept etcd/NATS/health traffic
...
# [FACT] Both nodes point to node1's etcd and NATS
...
MAX_MODEL_LEN=32768
MAX_NUM_SEQS=32
WORKER_READY_TIMEOUT=900   # [FACT] ~306s per worker, 900s gives safe headroom
...
# NIXL side channel ports — unique per prefill worker, across all nodes
NIXL_SIDE_CHANNEL_PORT_1=20097
NIXL_SIDE_CHANNEL_PORT_2=20098

# KV events ports — unique per prefill worker, across all nodes
KV_EVENTS_PORT_1=20081
KV_EVENTS_PORT_2=20082

# GPU assignments — same NUMA layout on both DGX B200 nodes
# [FACT] NUMA node 0: GPUs 0-3, CPUs 0-55,112-167
# [FACT] NUMA node 1: GPUs 4-7, CPUs 56-111,168-223
GPUS_DECODE="4,5,6,7"     # node1 only — NUMA node 1
GPUS_PREFILL_1="0,1,2,3"  # node1 — NUMA node 0
GPUS_PREFILL_2="0,1,2,3"  # node2 — NUMA node 0

# Results dir (NFS — visible from both nodes)
RESULTS_DIR="$HOME/software/vllm_efforts/benchmark_results"

# Benchmark scenarios
# [FACT] Ultra 1P:1D saturated at rate=10 (effective 2.17 req/s).
# rate=3 → unsaturated baseline; rate=10 → stress test to compare vs 1P:1D.
SCENARIOS=(
    #"16384,256,500,3,32"     # prefill-heavy, unsaturated — find λ* region
    #"16384,256,500,10,32"    # prefill-heavy, high load — compare vs 1P:1D
    #"16384,2048,200,3,32"   # decode-heavy, unsaturated — uncomment when ready
    "16384,2048,200,5,32"   # decode-heavy, high load  — uncomment when ready
)
...
run_node1() {
    local RUN_ID
    RUN_ID="$(tstamp)"
    local LOG_PREFIX="ULTRA_DISAGG_2P1D_2NODE"
    mkdir -p "$RESULTS_DIR"

    local FRONTEND_LOG="$RESULTS_DIR/${LOG_PREFIX}__frontend_${RUN_ID}.log"
    local DECODE_LOG="$RESULTS_DIR/${LOG_PREFIX}__decode1_${RUN_ID}.log"
    ...
    touch "$FRONTEND_LOG" "$DECODE_LOG" "$PREFILL1_LOG" "$ETCD_LOG" "$NATS_LOG"

    echo "=========================================="
    echo "Dynamo DISAGG 2P:1D Multi-Node — NODE1"
    ...
    echo "Decode-1:      GPUs $GPUS_DECODE  (DYN_SYSTEM_PORT $DECODE_SYSTEM_PORT,   NUMA 1)"
    echo "Prefill-1:     GPUs $GPUS_PREFILL_1 (DYN_SYSTEM_PORT $PREFILL_SYSTEM_PORT_1, NUMA 0)"
    echo "Prefill-2:     GPUs 0-3 on $NODE2_HOST — start same script on node2"
    ...
    cleanup_node1() {
        ...
        echo "NOTE: Kill node2 prefill-2 manually (Ctrl+C in its terminal)."
    }
    trap cleanup_node1 EXIT

    # --- Start etcd ---
    # [FACT] Must bind to 0.0.0.0 and advertise real IP so node2 can reach it.
    ...
    # Verify etcd is up on the real IP (not just loopback)
    local etcd_ok=0
    for i in {1..10}; do
    ...
    # --- Start decode worker (GPUs 4-7, NUMA 1) ---
    # [FACT] Decode starts first — prefill workers load decode metadata from etcd.
    ...
    # --- Start prefill worker 1 (GPUs 0-3, NUMA 0) ---
    ...
    echo "Waiting for node2 prefill-2 to come up..."
    echo "=========================================="
    echo ""

    # --- Wait for prefill-2 on node2 ---
    local p2_start=$SECONDS
    local p2_timeout=1200   # 20 min — generous for cold IB init
    local p2_ready=0
    ...
    [ "$p2_ready" -eq 1 ] || {
        echo "ERROR: prefill-2 on $NODE2_HOST not ready after ${p2_timeout}s" >&2
        exit 1
    }
    ...
    # [FACT] Extra sleep — frontend may return 200 before all workers fully registered
    echo "Allowing all workers to fully register in etcd (30s)..."
    sleep 30
    echo "All 3 workers registered. Ready to benchmark!"
    echo ""
    ...
    for SCENARIO in "${SCENARIOS[@]}"; do
        ...
        echo "=========================================="
        echo "Benchmarking (ULTRA DISAGG 2P:1D Multi-Node):"
        echo "  Input len:       $INPUT_LEN tokens"
        echo "  Output len:      $OUTPUT_LEN tokens"
        echo "  Num prompts:     $NUM_PROMPTS"
        echo "  Request rate:    $REQUEST_RATE req/s"
        echo "  Max concurrency: $MAX_CONCURRENCY"
        echo "  Topology:        2P:1D, TP=4, inter-node NIXL/IB"
        echo "  Time:            $(tstamp)"
        echo "=========================================="

        local RESULT_FILE
        RESULT_FILE="$RESULTS_DIR/ULTRA_DISAGG_2P1D_2NODE__in${INPUT_LEN}_out${OUTPUT_LEN}__prm${NUM_PROMPTS}__rate${REQUEST_RATE}__seq${MAX_CONCURRENCY}__${RUN_ID}.json"
    ...
    echo "=========================================="
    echo "All ULTRA DISAGG 2P:1D multi-node benchmarks complete!"
    echo "Results:"
    ls -lh "$RESULTS_DIR"/ULTRA_DISAGG_2P1D_2NODE__* 2>/dev/null || \
        echo "No result files found"
    echo "NOTE: Kill node2 prefill-2 with Ctrl+C in its terminal."
    echo "=========================================="
}
...
run_node2() {
    ...
    local PREFILL2_LOG="$RESULTS_DIR/ULTRA_DISAGG_2P1D_2NODE__prefill2_${RUN_ID}.log"
    touch "$PREFILL2_LOG"

    echo "=========================================="
    echo "Dynamo DISAGG 2P:1D Multi-Node — NODE2"
    echo "Dynamo version: 1.3.0-dev.1"
    echo "=========================================="
    echo "Run ID:         $RUN_ID"
    echo "Model:          $MODEL"
    ...
    echo "NIXL SC port:   $NIXL_SIDE_CHANNEL_PORT_2"
    echo "KV events port: $KV_EVENTS_PORT_2"
    echo "Log:            $PREFILL2_LOG"
    echo "Time:           $(tstamp)"
    echo "=========================================="
    ...
    # Verify etcd on node1 is reachable before we do anything
    ...
    # --- Start prefill worker 2 (GPUs 0-3, NUMA 0) ---
    # [FACT] ETCD_ENDPOINTS and NATS_SERVER point to node1 — set in env above
    #        and passed explicitly here for clarity.
    # [FACT] kv-events endpoint tcp://*:20082 binds on all interfaces on node2.
    #        NIXL registers the reachable address in etcd; decode on node1
    #        performs RDMA pull of KV cache over IB.
    ...
    python3 -m dynamo.vllm \
        ...
        > "$PREFILL2_LOG" 2>&1 &
    PREFILL2_PID=$!
    echo "Prefill worker 2 PID: $PREFILL2_PID"

    # Wait for prefill-2 to be locally ready
    wait_for_worker \
        "http://127.0.0.1:${PREFILL_SYSTEM_PORT_2}/health" \
        "prefill worker 2"

    echo ""
    echo "=========================================="
    echo "PREFILL-2 READY AND REGISTERED IN ETCD."
    ...
    # Stay alive and stream log for visibility
    tail -f "$PREFILL2_LOG"

    # cleanup_node2() kills prefill-2 on EXIT
}
