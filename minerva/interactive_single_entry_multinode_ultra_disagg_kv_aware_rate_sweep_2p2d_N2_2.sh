...
# Workers load from local snapshot path (no download attempt).
# Benchmark client and --served-model-name use canonical HF ID so any
# client using the HF model ID string can still reach the server.
...
# -------------------------------------------------------------------------
# ISL=131,072  OSL=256  max_model_len=131,328 (ISL+OSL — fixed for all steps)
#
...
#
# -------------------------------------------------------------------------
# OSL=256 RATIONALE (from §26 of PROJECT_MEMORY)
# -------------------------------------------------------------------------
# [LOGICAL DEDUCTION] λ* = N_P / T_prefill — OSL does NOT appear.
# OSL=256 → decode time ≈ 256 × 7ms = 1.8s — negligible vs TTFT ≈ 12s.
...
RATE_SWEEP=(
    "131072,256,32,20,0.05,4"
    "131072,256,32,20,0.10,4"
    "131072,256,32,20,0.20,8"
    "131072,256,32,20,0.50,16"
    "131072,256,32,20,1.00,32"
)
...
mkdir -p "$RESULTS_DIR"
RUN_ID="$(tstamp)"
SWEEP_SUMMARY="$RESULTS_DIR/ULTRA_DISAGG_2P2D_KV_2NODE__rate_sweep_summary_${RUN_ID}.txt"

log "=========================================="
log "Nemotron Ultra NVFP4 — DISAGG 2P:2D Rate Sweep (ISL=131072, OSL=256 fixed)"
...
log "ISL: 131072 (fixed), OSL: 256 (fixed), max_model_len: 131328 (fixed)"
...
echo "ISL=131072, OSL=256 (fixed), TTFT_LIMIT=${TTFT_LIMIT_MS}ms" >> "$SWEEP_SUMMARY"
...
for STEP in "${RATE_SWEEP[@]}"; do
    ...
    # ------------------------------------------------------------------
    STEP_FRONTEND_LOG="$RESULTS_DIR/ULTRA_DISAGG_2P2D_KV_2NODE__frontend__RATE${RATE}__${RUN_ID}.log"
