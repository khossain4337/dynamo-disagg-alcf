...
RATE_SWEEP=(
    # Format: "NUM_PROMPTS,REQUEST_RATE,MAX_CONCURRENCY"
    # OSL=8192, ISL=131072, T_total ≈ 8.6s + 8192×7ms ≈ 65.9s
    # Little's Law: N = λ × T_total
    #   rate=0.05 → N≈3.3   MAX_CONCURRENCY=8   (headroom, cold run)
    #   rate=0.10 → N≈6.6   MAX_CONCURRENCY=10
    #   rate=0.20 → N≈13.2  MAX_CONCURRENCY=16
    #   rate=0.50 → N≈33.0  MAX_CONCURRENCY=32  (at max-num-seqs wall — keep to observe cap)
    # rate=1.00 DROPPED: N≈66 >> max-num-seqs=32 → cap dominates, confounds AGG vs DISAGG
    "20,0.05,8"
    "20,0.10,10"
    "20,0.20,16"
    "20,0.50,32"
...
mkdir -p "$RESULTS_DIR"
RUN_ID="$(tstamp)"
SWEEP_SUMMARY="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__rate_sweep_summary_${RUN_ID}.txt"
...
# =============================================================================
ETCD_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__etcd_${RUN_ID}.log"
NATS_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__nats_${RUN_ID}.log"
FRONTEND_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__frontend_${RUN_ID}.log"
...
# =============================================================================
W0_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__worker0_${RUN_ID}.log"
W1_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__worker1_${RUN_ID}.log"
W2_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__worker2_${RUN_ID}.log"
W3_LOG="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__worker3_${RUN_ID}.log"
...
#   tail -f $RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__worker{2,3}_<RUN_ID>.log
...
for STEP in "${RATE_SWEEP[@]}"; do
    ...
    RESULT_FILE="$RESULTS_DIR/ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__in${ISL}_out${OSL}__prm${NUM_PROMPTS}__rate${RATE}__${RUN_ID}.json"
...
v = p.get('P99') or p.get('p99') or d.get('p99_ttft_ms') or 0
print(f'{v:.1f}')
" 2>/dev/null || echo "0")
...
v = p.get('P99') or p.get('p99') or d.get('p99_tpot_ms') or 0
print(f'{v:.1f}')
" 2>/dev/null || echo "0")
...
log "Worker logs in:       $RESULTS_DIR (ULTRA_AGG_4xTP4_RR_2NODE_OSL8192__*.log)"
log "=========================================="
log ""
log "=========================================="
log "CLEANUP: Shutting down all workers..."
